...
if (isset($_POST['submitdoc'])) {
    ...
    function validateLink($url) {
        return preg_match('/^https:\/\/(docs|sheets|slides)\.google\.com\/(document|spreadsheets|presentation)\/d\/[a-zA-Z0-9_-]+/', $url);
    }
    ...
    function getDocType($url) {
        if (strpos($url, 'docs.google.com/document') !== false) {
            return 'document';
        } elseif (strpos($url, 'sheets.google.com/spreadsheets') !== false) {
            return 'sheet';
        } elseif (strpos($url, 'slides.google.com/presentation') !== false) {
            return 'slide';
        } else {
            return false;
        }
